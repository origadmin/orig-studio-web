...
import {useNavigate, useMatchRoute, useLocation} from '@tanstack/react-router';
...
const ProfileLayout: React.FC<ProfileLayoutProps> = ({username}) => {
    ...
    const ownerTabs = [
        {key: 'videos', icon: Video, label: t('nav.myVideos'), path: `/@${username}`},
        {key: 'channels', icon: Tv, label: t('nav.myChannels'), path: `/@${username}/channels`},
        ...(modules.articles ? [{key: 'articles', icon: FileText, label: t('nav.myArticles'), path: `/@${username}/articles`}] : []),
        {key: 'playlists', icon: ListVideo, label: t('nav.myPlaylists'), path: `/@${username}/playlists`},
        {key: 'favorites', icon: Heart, label: t('nav.myFavorites'), path: `/@${username}/favorites`},
        {key: 'history', icon: History, label: t('nav.history'), path: `/@${username}/history`},
        {key: 'about', icon: Info, label: t('profile.tabAbout'), path: `/@${username}/about`},
    ];

    const visitorTabs = [
        {key: 'videos', icon: Film, label: t('profile.tabVideos'), path: `/@${username}`},
        {key: 'playlists', icon: ListVideo, label: t('profile.tabPlaylists'), path: `/@${username}/playlists`},
        {key: 'about', icon: Info, label: t('profile.tabAbout'), path: `/@${username}/about`},
    ];
    ...
    const activeTab = tabs.find(tab => {
        if (tab.key === 'videos') {
            return currentPath === `/@${username}` || currentPath === `/@${username}/`;
        }
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-4 gap-2">
                            <a
                                href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(profile?.nickname || username)}`}
                                target="_blank"
                            ...
                            <a
                                href={`https://t.me/share/url?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(profile?.nickname || username)}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex flex-col items-center gap-1 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                            >
