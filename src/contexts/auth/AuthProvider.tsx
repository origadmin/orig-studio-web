...
async function refreshUserFromMe(storedUser: User): Promise<User> {
    try {
        // BUG-088: /me goes through the same axios interceptor as every other
        // business endpoint — no special-casing. 401 handling is unified in
        // request.ts (refresh + replay + _retry guard + global counter).
        // The refresh loop was broken at the call site (Fix 2): scheduled
        // refresh no longer calls refreshUserFromMe, so there is no cascade.
