...
export const mediaApi = {
    ...
    likes: {
        ...
        // 点踩/取消点踩（独立端点；原先误接 DELETE /likes=unlike，无法产生 dislike）
        toggleDislike: (token: string | number) =>
            api.post<LikeResponse>(`/medias/${token}/dislikes`),
    },
...
export const publicMediaApi = {
    ...
    // BUG-105: 传入 update_mask（AIP-134 FieldMask）时启用掩码语义——protobuf JSON
    // 中 FieldMask 编码为逗号分隔的 camelCase 路径字符串（如 "channelId"）；
    // 含 channel_id 且值为空串即表示"移到无频道（清空归属）"；不传则走 legacy 全量合并。
    update: (shortToken: string, data: UpdateMediaRequest, updateMask?: string[]) =>
        api.put<Media>(`/medias/${shortToken}`, updateMask && updateMask.length > 0
            ? {media: data, update_mask: updateMask.join(',')}
            : {media: data}),
